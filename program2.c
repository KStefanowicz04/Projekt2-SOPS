...
// Mutex, który blokuje czytelników przed wejściem do czytelni ("bramka")
pthread_mutex_t reader_gate = PTHREAD_MUTEX_INITIALIZER;
...
void* reader(void* arg) {
    while(1) {
        ...
        // "bramka" - Czytelnik jest blokowany w tym miejscu jeśli bramka została już zablokowana przez Pisarza 
        pthread_mutex_lock(&reader_gate);
        pthread_mutex_unlock(&reader_gate);
...
void* writer(void* arg) {
    while(1) {
        ...
        // Pisarz blokuje bramkę dla czytelników
        pthread_mutex_lock(&reader_gate);
        // i czeka aż czytelnia będzie wolna
        pthread_mutex_lock(&room_empty);
        ...
        pthread_mutex_unlock(&room_empty);
        // oraz odblokowuje bramkę dla czytelników
        pthread_mutex_unlock(&reader_gate);
